...
function VizInsights() {
  // Course-enrollment bar chart + GPA distribution overlay
  const bars = [38, 52, 71, 88, 64, 49, 35, 42, 58, 76, 81, 60];
  return (
    <div className="viz">
      <div className="viz-hd"><span>UW · CS200 series enrollment</span><span>2018–25</span></div>
      <svg viewBox="0 0 320 150">
        {bars.map((h, i) => (
          <rect key={i} x={22 + i * 24} y={130 - h} width={16} height={h}
                fill="var(--accent)" opacity={0.18 + (i / bars.length) * 0.55}/>
        ))}
        {/* GPA distribution overlay */}
        <path d={`M 20 90 ${bars.map((h, i) => `L ${30 + i * 24} ${110 - Math.sin(i * 0.7) * 12 - i * 0.6}`).join(' ')}`}
              stroke="var(--semantic-info)" strokeWidth="1.5" fill="none" opacity=".7"/>
        <g fontFamily="var(--font-mono)" fontSize="9" fill="var(--fg-muted)">
          <text x="22" y="144">'18</text>
          <text x="305" y="144" textAnchor="end">'25</text>
        </g>
      </svg>
    </div>
  );
...
function VizScout() {
  ...
  return (
    <div className="viz">
      <div className="viz-hd"><span>ScoutEdge · xG heatmap</span><span>90'</span></div>
      <svg viewBox="0 0 320 150">
...
const FEATURED = [
  {
    ...
    type: 'Full-Stack',
    status: 'Live',
    summary: 'Personal-finance dashboard that maps spending into intent-based buckets, with predictive monthly burn and an LLM categorization layer for messy bank descriptions.',
    Viz: VizMoneyMap,
    highlights: [
      { metric: '92%', text: 'auto-categorization accuracy across 4k+ transactions, using Claude + rules.' },
      { metric: '< 800ms', text: 'p95 dashboard load via SWR + Prisma aggregations on Postgres.' },
      { metric: 'CSV → JSON', text: 'idempotent import pipeline handling Chase, BofA, and KB statements.' },
    ],
    stack: ['Next.js', 'TypeScript', 'Prisma', 'Postgres', 'Claude API', 'Tailwind'],
    repo: 'https://github.com/suheum-heo/moneymap',
    demo: 'https://moneymap-io.vercel.app/',
  },
  {
    title: 'ScoutEdge',
    role: 'Solo · AI / Sports analytics',
    type: 'AI',
    status: 'In progress',
    summary: 'Football scouting tool that ingests StatsBomb open data, generates xG/xT heatmaps, and uses an LLM agent to produce 1-paragraph scouting reports grounded in retrieved match events.',
    Viz: VizScout,
    highlights: [
      { metric: '12k+', text: 'shot events processed; per-match xG model with sklearn pipelines.' },
      { metric: 'RAG', text: 'event-grounded reports — every claim cites a minute and a match ID.' },
      { metric: 'Streamlit', text: 'interactive shot map + filterable player comparison view.' },
    ],
    stack: ['Python', 'Pandas', 'scikit-learn', 'Streamlit', 'Plotly', 'Claude API'],
    repo: 'https://github.com/suheum-heo/scout-edge',
    demo: 'https://scout-edge.vercel.app/',
  },
];
...
const PROJECTS = [
  {
    ...
    type: 'Data',
    summary: 'Hourly ingest of OHLCV + fundamentals for S&P 500 tickers, deduped and warehoused in SQLite/Postgres with retry-safe scheduling.',
    Viz: VizStock,
  ...
  },
  {
    title: 'Common Ground',
    type: 'AI',
    summary: 'LLM-powered debate visualizer that maps opposing arguments into a shared graph and surfaces points of agreement.',
    Viz: VizCommonGround,
    stack: ['Next.js', 'Claude API', 'D3', 'Supabase'],
    award: 'Anthropic × CDIS Hackathon · Finalist',
...
const SKILLS = [
  ...
  },
  {
    ...
    icon: 'back',
    tag: '08',
    items: ['Node.js', 'Express', 'Flask', 'FastAPI', 'PostgreSQL', 'SQLite', 'Prisma', 'Supabase'],
  },
  {
    key: 'ai',
    ...
    icon: 'ai',
    tag: '06',
    items: ['Pandas', 'NumPy', 'scikit-learn', 'Matplotlib', 'Plotly', 'Anthropic Claude API'],
  },
];
